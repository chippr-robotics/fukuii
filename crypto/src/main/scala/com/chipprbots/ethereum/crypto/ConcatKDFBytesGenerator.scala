...
/** Basic KDF generator for derived keys and ivs as defined by NIST SP 800-56A.
  * @param digest
  *   for source of derived keys
  */
class ConcatKDFBytesGenerator(digest: Digest) {
  val digestSize: Int = digest.getDigestSize

  /** @param outputLength
    *   length of output that will be produced by this method, maximum value is (digest output size in bits) * (2^32 -
    *   1) but it should not be a problem because we are using Int
    * @throws scala.IllegalArgumentException
    *   ("Output length too large") when outputLength is greater than (digest output size in bits) * (2^32 - 1)
    * @return
    *   returns bytes generated by key derivation function
    */
