...
object PairingCheck {
  ...
  /** Pairing function is defined as: `e: G_1 x G_2 -> G_T` where G1 is element of
    * [[com.chipprbots.ethereum.crypto.zksnark.BN128.BN128G1]] and G2 is element of
    * [[com.chipprbots.ethereum.crypto.zksnark.BN128.BN128G2]]
    *
    ...
    *
    * @param pairs
    *   Sequence of valid pairs of group elements (a1, b1, a2, b2, ..., ak, bk) from (G_1 x G_2)^k^
    * @return
    *   `true` if `log_P1(a1) * log_P2(b1) + ... + log_P1(ak) * log_P2(bk) = 0` else `false`
    */
