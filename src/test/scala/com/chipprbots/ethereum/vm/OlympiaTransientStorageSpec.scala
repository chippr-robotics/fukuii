...
/** Tests for EIP-1153: Transient Storage Opcodes (TLOAD 0x5c, TSTORE 0x5d)
  * https://eips.ethereum.org/EIPS/eip-1153
  *
  * TSTORE: pop key and value from stack, store in transient storage scoped to (address, key)
  * TLOAD: pop key from stack, push value from transient storage (or 0 if not set)
...
class OlympiaTransientStorageSpec extends AnyWordSpec with Matchers {
  ...
  object fxt {
    ...
    val codeOverwrite: Assembly = Assembly(
      PUSH1, 42,    // first value
      PUSH1, 0,     // key
      TSTORE,
      PUSH1, 99,    // second value (overwrite)
      PUSH1, 0,     // same key
      TSTORE,
      PUSH1, 0,     // key
