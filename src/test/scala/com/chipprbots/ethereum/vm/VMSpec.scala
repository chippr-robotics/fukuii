...
class VMSpec extends AnyWordSpec with ScalaCheckPropertyChecks with Matchers {
  ...
  "VM" when {
    ...
    "executing contract creation" should {
      ...
      "go OOG if new contract's code size exceeds limit and block is after atlantis or eip161" taggedAs(UnitTest, VMTest) in new ContractCreation {
      ...
      "fail to create contract in case of address conflict (non-empty code)" taggedAs(UnitTest, VMTest) in new ContractCreation {
      ...
      "fail to create contract in case of address conflict (non-zero nonce)" taggedAs(UnitTest, VMTest) in new ContractCreation {
      ...
      "create contract if the account already has some balance, but zero nonce and empty code" taggedAs(UnitTest, VMTest) in new ContractCreation {
      ...
      "initialise a new contract account with zero nonce before EIP-161" taggedAs(UnitTest, VMTest) in new ContractCreation {
      ...
      "initialise a new contract account with incremented nonce after EIP-161" taggedAs(UnitTest, VMTest) in new ContractCreation {
