      ...
      case Right(nextMsg) =>
        nextMsg.messageToSend match
          case statusEnc: ETHPackets.Status68.Status68.Status68Enc =>
            ...
            val expectedForkId = ForkId.create(genesisBlock.header.hash.value, genesisBlock.header.unixTimestamp.toLong, blockchainConfig)(lowBlockNumber)
      ...
      case Right(nextMsg) =>
        nextMsg.messageToSend match
          case statusEnc: ETHPackets.Status68.Status68.Status68Enc =>
            ...
            val expectedForkId = ForkId.create(genesisBlock.header.hash.value, genesisBlock.header.unixTimestamp.toLong, blockchainConfig)(highBlockNumber)
            statusMsg.forkId shouldBe expectedForkId
          case other =>
            fail(s"Expected ETHPackets.Status68.Status68Enc message but got: $other")
      case other =>
