...
/** Multi-step feedback loop tests for PeerScore.
  ...
  * matter for the SNAP sync peer-selection feedback loop: score degrades monotonically on consecutive failures, recovers
  * after successes, and shouldRetry caps at 1 hour regardless of blacklist count.
  */
class PeerScoreFeedbackSpec extends AnyFlatSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  "PeerScore timeout feedback" should "degrade score monotonically with each timeout" taggedAs
    ...
    val scores = (1 to 5).scanLeft(initial) { (s, _) => s.recordTimeout }
      .map(_.score)
  ...
  it should "have a lower score after 10 timeouts than after 1 timeout" taggedAs (UnitTest, NetworkTest) in {
    ...
    val after1 = base.recordTimeout
    val after10 = (1 to 10).foldLeft(base)((s, _) => s.recordTimeout)
    after10.score should be < after1.score
  }
