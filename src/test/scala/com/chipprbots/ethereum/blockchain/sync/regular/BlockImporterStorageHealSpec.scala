...
/** Unit-level tests pinning the BlockImporter Tier 2 storage escalation logic.
  *
  * These tests verify the GUARD CONDITIONS for the new storage heal path added
  * in commits 880ab457d (scaffold) and e25410946 (immediate escalation).
  *
  * Actor-level integration (resolvingStorageRange state machine transitions)
  * is covered by run-level observation — the unit tests here pin the key
  * invariants that don't require a full actor setup.
  */
class BlockImporterStorageHealSpec extends AnyFlatSpec with Matchers {
  ...
  "Tier 2 escalation guard" should "fire on first exhaust when a storage account is tracked" taggedAs (UnitTest, SyncTest) in {
  ...
  "MaxStuckDurationMs" should "be large enough to survive normal slow healing (>= 15 minutes)" taggedAs (UnitTest, SyncTest) in {
    MaxStuckDurationMs should be >= (15L * 60L * 1000L) // 15 minutes
  }

  "StuckEscapeThreshold" should "be 3 (3 consecutive exhausts for non-storage nodes before escalation)" taggedAs (UnitTest, SyncTest) in {
    StuckEscapeThreshold shouldBe 3
  }

  // ─── FetchAccountStorage message type invariants ─────────────────────────────

  "FetchAccountStorage message" should "carry accountAddress, replyTo, canonicalStateRoot, and stateStorage" taggedAs (UnitTest, SyncTest) in {
  ...
  "FetchedAccountStorage message" should "distinguish success from failure via the success flag" taggedAs (UnitTest, SyncTest) in {
    val successMsg = BlockFetcher.FetchedAccountStorage(testAccountAddr, Some(com.chipprbots.ethereum.domain.Account()), success = true)
