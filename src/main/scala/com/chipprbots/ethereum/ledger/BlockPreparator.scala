...
class BlockPreparator(
  ...
  /** ECIP-1111: Credit baseFee revenue to the treasury address.
    * This runs AFTER block rewards and ommer rewards, matching core-geth's Finalize() order.
    */
  ...
  /** EIP-7702: Process authorization list, setting delegation codes on authorized accounts.
    * Each authorization is validated and applied independently; failures are silently skipped.
    */
    ...
    if (auth.chainId != 0 && auth.chainId != blockchainConfig.chainId) None
      ...
      authority.flatMap { authorityAddr =>
        ...
        if (code.nonEmpty && !SetCodeTransaction.isDelegation(code)) None
          ...
          val account = world.getAccount(authorityAddr)
...
object BlockPreparator {
  ...
  /** EIP-7623: Calculate floor data gas for a transaction.
    * Floor ensures calldata-heavy transactions pay a minimum gas cost.
