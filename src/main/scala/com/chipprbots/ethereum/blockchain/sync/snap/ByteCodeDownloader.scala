...
class ByteCodeDownloader(
  ...
  def requestNextBatch(peer: Peer): Option[BigInt] = synchronized {
    ...
    log.debug(s"Requesting ${task.codeHashes.size} bytecodes ${task.taskString} from peer ${peer.id} (request ID: $requestId)")
  ...
  /** Handle ByteCodes response
    ...
    * @return Either error message or number of bytecodes processed
    */
  ...
  /** Process downloaded bytecodes
    ...
    * @return Number of bytecodes processed
    */
  ...
  /** Verify that downloaded bytecodes match their expected hashes
    *
    * @param expectedHashes Expected code hashes
    * @param bytecodes Downloaded bytecodes
    ...
    val mismatches = bytecodes.zipWithIndex.collect {
      case (code, idx) if idx < expectedHashes.size =>
        ...
        if (actualHash != expectedHash) {
          Some(s"Index $idx: expected ${expectedHash.take(4).toArray.map("%02x".format(_)).mkString}, " +
