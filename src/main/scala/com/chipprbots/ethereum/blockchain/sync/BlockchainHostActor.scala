...
class BlockchainHostActor(
  ...
  /** Handles ETH65 GetPooledTransactions requests by looking up the requested hashes in the tx pool. Response is sent
    * asynchronously via networkPeerManagerActor once the pool responds.
    */
