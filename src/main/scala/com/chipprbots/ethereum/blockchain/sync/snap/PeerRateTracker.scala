...
/** Per-peer message rate tracker for adaptive SNAP sync timeouts.
  *
  * Port of geth's p2p/msgrate package. Tracks per-peer throughput (items/sec) and round-trip times,
  * then computes adaptive request timeouts and per-peer request capacities.
  *
  * Algorithm:
...
class PeerRateTracker extends Logger {
  ...
  /** Record a measurement after receiving a response from a peer.
    ...
    * @param msgType   SNAP message type ordinal
    * @param elapsedMs time from request send to response receive (milliseconds)
    * @param items     number of items in the response (0 = timeout/failure)
    */
  ...
  /** Calculate the maximum number of items to request from a peer.
    ...
    * @param msgType    SNAP message type ordinal
    * @param targetRTT  target round-trip time in milliseconds (from targetRoundTrip())
    * @return           number of items to request (minimum 1)
    */
  ...
  /** Calculate the adaptive timeout for SNAP requests.
    ...
    *
    * @return timeout as FiniteDuration
    */
  ...
  /** Calculate the target round-trip time (pushed down for efficiency).
    *
    * @return target RTT in milliseconds
    */
  ...
  /** Recalculate median RTT and update confidence. Call periodically (every ~5 seconds).
    *
    * Geth calls tune() every medianRTT interval. We simplify to a fixed interval since
    * actor scheduling is more natural with fixed periods.
    */
