...
/** LRU cache for storage tries to limit memory usage
  *
  * LinkedHashMap maintains insertion order. For LRU, we move accessed items to end. When cache is full, we evict the
...
/** Storage Range Downloader for SNAP sync
  ...
  * locally. Follows core-geth snap sync patterns.
  *
  * Features:
  ...
  *
  * Unlike AccountRangeDownloader which downloads ranges of accounts, this downloader works on a per-account basis,
  * downloading all storage slots for each account.
  *
  * @param stateRoot
  *   State root hash (for context, not directly used)
  * @param networkPeerManager
  *   Actor for peer communication
  * @param requestTracker
  *   Request/response tracker
  * @param mptStorage
  *   Storage for persisting downloaded storage slots
  * @param maxAccountsPerBatch
  *   Maximum accounts to request storage for in a single request (default 8)
  */
class StorageRangeDownloader(
  ...
  /** Request next batch of storage ranges from available peer
    *
    * Batches multiple account storage requests together to reduce message overhead. Follows core-geth pattern of
    * requesting storage for multiple accounts at once.
    *
    * @param peer
  ...
  /** Handle StorageRanges response
    ...
    * @return
    *   Either error message or number of slots processed
    */
  ...
  /** Process downloaded storage ranges
    *
    * StorageRanges response contains a list of slot sets, one per account. Match them up with the requested tasks and
    * verify/store each.
    *
    * @param tasks
    *   The tasks being filled
    * @param response
    *   The validated response
    * @return
    *   Number of slots processed across all accounts
    */
            ...
            case Right(_) =>
              ...
              if (accountSlots.nonEmpty) {
                ...
                if (lastSlot.toSeq.compare(task.last.toSeq) < 0) {
                  ...
                  log.debug(
                    s"Created continuation task for account ${task.accountString} from ${continuationTask.rangeString}"
                  )
                }
              }
  ...
  /** Store storage slots to MptStorage
    *
    * Inserts storage slots into per-account storage tries using proper MPT structure. Thread-safe storage with
    * synchronized access for concurrent task writes.
    *
    ...
    *
    * @param accountHash
    *   Hash of the account owning this storage
    * @param slots
    *   Storage slots to store (slotHash -> slotValue)
    * @return
      ...
      this.synchronized {
        if (slots.nonEmpty) {
          ...
          storageTaskOpt match {
            case None =>
              log.warn(
                s"No storage task found for account ${accountHash.take(4).toArray.map("%02x".format(_)).mkString}"
              )
            ...
            case Some(storageTask) =>
              ...
              val storageTrie = storageTrieCache.getOrElseUpdate(
                accountHash, {
                  ...
                  if (storageRoot.isEmpty || storageRoot == ByteString(MerklePatriciaTrie.EmptyRootHash)) {
                    ...
                    try {
                      log.debug(
                        s"Loading storage trie for account ${accountHash.take(4).toArray.map("%02x".format(_)).mkString}"
                      )
              ...
              if (computedRoot != expectedRoot) {
                log.warn(
                  s"Storage root mismatch for account ${accountHash.take(4).toArray.map("%02x".format(_)).mkString}"
                )
  ...
  /** Get or create proof verifier for a storage root
    ...
    *   Storage root hash
    * @return
    *   Verifier for this storage root
    */
