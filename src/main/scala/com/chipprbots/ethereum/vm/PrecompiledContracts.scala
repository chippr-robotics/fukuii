...
object PrecompiledContracts {
      ...
      if (etcFork >= EtcForks.Olympia) {
        ...
        if (baseLength > maxOperandLength || expLength > maxOperandLength || modLength > maxOperandLength) {
          return ProgramResult(
            ByteString.empty, BigInt(0), context.world, Set.empty, Nil, Nil, 0,
            Some(PreCompiledContractFail), Set.empty, Set.empty
          )
        }
      }
  ...
  private def blsG1MsmDiscount(k: Int): Int = {
    val table = Array(
      1000, 949, 848, 797, 764, 740, 721, 707, 695, 685,
       677, 670, 664, 659, 654, 650, 646, 643, 640, 637,
       634, 632, 630, 627, 625, 624, 622, 620, 618, 617,
       ...
       524, 523, 522, 521, 520, 519, 519, 519
    )
    if (k <= 0) 1000
  ...
  private def blsG2MsmDiscount(k: Int): Int = {
    val table = Array(
      1000, 1000, 923, 884, 855, 832, 812, 796, 782, 770,
       759, 750, 742, 734, 727, 721, 715, 709, 704, 699,
       694, 689, 685, 681, 677, 673, 669, 666, 662, 659,
       656, 653, 650, 647, 644, 641, 639, 636, 634, 631,
       629, 627, 624, 622, 620, 618, 616, 614, 612, 610,
       608, 606, 604, 603, 601, 599, 597, 596, 594, 593,
       591, 590, 588, 587, 585, 584, 582, 581, 580, 578,
       577, 576, 574, 573, 572, 571, 569, 568, 567, 566,
       565, 564, 563, 562, 561, 560, 559, 558, 557, 556,
       555, 554, 553, 552, 551, 550, 549, 548, 547, 547,
       546, 545, 544, 543, 543, 542, 541, 540, 540, 539,
       538, 537, 537, 536, 535, 535, 534, 533, 533, 532,
       531, 531, 530, 530, 529, 528, 528, 527
    )
    if (k <= 0) 1000
    else if (k <= table.length) table(k - 1)
    else 524 // for k > 128
