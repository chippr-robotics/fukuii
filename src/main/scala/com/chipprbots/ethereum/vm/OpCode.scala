...
/** EIP-3198: BASEFEE opcode — pushes the block's baseFee onto the stack.
  * Returns 0 for pre-Olympia blocks where baseFee is not set.
  */
...
/** EIP-1153: TSTORE — store to transient storage. Gas: G_warm_storage_read (100).
...
/** EIP-5656: MCOPY — memory-to-memory copy with proper overlap handling.
  * Gas: G_verylow (3) + 3 * ceil(size/32) + memory expansion cost.
  */
