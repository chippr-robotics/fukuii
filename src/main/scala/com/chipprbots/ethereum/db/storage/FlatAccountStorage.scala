...
/** Flat storage for accounts — O(1) reads by keccak256(address).
  ...
  * keccak256(address) (32 bytes) in a dedicated RocksDB column family. Mirrors
  * Geth's flat snapshot layer ("a" prefix convention).
  *
  * Benefits:
...
class FlatAccountStorage(val dataSource: DataSource) extends TransactionalKeyValueStorage[ByteString, ByteString] {
  ...
  /** Seek-based range scan starting from startHash. Returns ordered (hash, rlpAccount) pairs.
