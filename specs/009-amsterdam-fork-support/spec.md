...
- **FR-017**: The gas figures observed in the reference fixture MUST be reproduced exactly by the implementation, including the two currently-unexplained cases (see Assumptions), before this feature is considered complete. A figure that cannot be derived is a gap in understanding, not a rounding difference.
...
- **Fork scope is defined by the reference fixture.** The EIPs treated as in scope are those the devp2p fixture actually exercises: resource-based intrinsic gas, state-access repricing, state-creation gas with its reservoir model, block gas accounting, block-level access lists, slot number, builder execution requests, and raised code size limits. Any further Amsterdam EIP not exercised by that fixture is out of scope for this feature and should be specified separately.
- **Two gas figures are not yet explained and must be closed during planning, not assumed.** First, the state-creating transaction's Amsterdam total appears as exactly 183,600 rather than prior execution gas plus a state-creation charge. Second, four transaction variants that previously cost between roughly 49,000 and 52,000 all collapse to exactly their 100,000 gas limit, i.e. they run out of gas. Both plausibly follow from the interaction of the state-creation reservoir with the revised block gas accounting, but that is inference from specification text, not a reproduction. FR-017 makes closing this an explicit completion gate.
