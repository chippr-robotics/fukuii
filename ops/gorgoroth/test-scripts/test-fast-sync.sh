...
RUNNING=$($DOCKER_COMPOSE -f docker-compose-6nodes.yml ps -q fukuii-node1 fukuii-node2 fukuii-node3 | wc -l)
if [ "$RUNNING" -eq 3 ]; then
  log_info "Seed nodes already running, skipping startup"
else
  $DOCKER_COMPOSE -f docker-compose-6nodes.yml up -d fukuii-node1 fukuii-node2 fukuii-node3
...
$DOCKER_COMPOSE -f docker-compose-6nodes.yml stop fukuii-node6 2>/dev/null || true
sleep 5

# Clean node 6 data to ensure fresh fast sync
log_info "Cleaning node 6 data directory..."
$DOCKER_COMPOSE -f docker-compose-6nodes.yml rm -f fukuii-node6 2>/dev/null || true

# Start fresh node 6
log_info "Starting node 6 with fast sync enabled..."
$DOCKER_COMPOSE -f docker-compose-6nodes.yml up -d fukuii-node6
...
if [ "$SYNCED" = true ]; then
  ...
  $DOCKER_COMPOSE -f docker-compose-6nodes.yml logs --tail=20 fukuii-node6
  exit 1
fi
...
DECOMPRESS_ERRORS=$($DOCKER_COMPOSE -f docker-compose-6nodes.yml logs fukuii-node6 2>&1 | \
...
if [ $DECOMPRESS_ERRORS -eq 0 ]; then
  ...
  echo
  echo "Sample errors:"
  $DOCKER_COMPOSE -f docker-compose-6nodes.yml logs fukuii-node6 2>&1 | \
